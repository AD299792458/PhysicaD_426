# Program to generate data for Figure 8: Simulation of coherence resonance in postsynaptic neurons.
import numpy
...
class FHN():
	def __init__(self, params):
		...
		self.p = params[14]		# Dummy parameter to be used in future work to simulate probabilistic firing.
	...
	def euler_maruyama(self):
		...
		flags = numpy.full(self.N, False)	# Array of flags to track whether presynaptic neurons have crossed lower threshold.

		for i in range(int(self.T/self.dt)-1):
			dW = numpy.random.normal(0, 1, self.N)
			self.v=self.v + (self.v*(self.v-self.a)*(1-self.v)-self.w)*self.dt/self.eps + (self.sigma/self.eps)*numpy.sqrt(self.dt)*dW
...
# Class for postsynaptic neurons.
class postFHN():
	def __init__(self, params):
		self.eps = params[0]
		self.a = params[1]
	...
	def RK4(self):
		...
		for i in range(len(self.v)-1):
			k1 = (1/self.eps)*(self.v[i]*(self.v[i]-self.a)*(1-self.v[i]) - self.w[i] + self.synapse[i])
			l1 = self.v[i] - self.w[i] - self.b
			k2 = (1/self.eps)*((self.v[i]+self.dt*k1/2)*(self.v[i]+self.dt*k1/2-self.a)*(1-self.v[i]-self.dt*k1/2)-(self.w[i]+self.dt*l1/2)+ self.synapse[i])
			l2 = (self.v[i] + dt*k1/2) - (self.w[i] + dt*l1/2) - self.b
			k3 = (1/self.eps)*((self.v[i]+self.dt*k2/2)*(self.v[i]+self.dt*k2/2-self.a)*(1-self.v[i]-self.dt*k2/2)-(self.w[i]+self.dt*l2/2)+ self.synapse[i])
			l3 = (self.v[i] + self.dt*k2/2) - (self.w[i] + self.dt*l2/2) - self.b
			k4 = (1/self.eps)*((self.v[i]+self.dt*k3)*(self.v[i]+self.dt*k3-self.a)*(1-self.v[i]-self.dt*k3)-(self.w[i]+self.dt*l3)+ self.synapse[i])
...
b = 0.15
I = 0
A = 0	# No sinusoidal stimulus: Presynaptic neurons are stimulated by (gaussian, white) noise alone.
f =0	# No sinusoidal stimulus: Presynaptic neurons are stimulated by (gaussian, white) noise alone.
dt = 0.001
T = 1000.0
v0 = 0.11151
w0 = -0.03849 
...
# Initialize objects for pre and postsynaptic neurons respectively.
neuron = FHN([eps, a, b, I, A, f, dt, T, v0, w0, N, height, width, 0, p])
post = postFHN([eps, a, b, I, dt, T, v0, w0])

cv = numpy.zeros(len(sigma)) 	# Array to store coefficient of variation of interspike intervals for each noise intensity.

for i in range(len(sigma)):
	print(str(i+1)+'/'+str(len(sigma)))
	neuron.sigma = sigma[i]		# Set noise for presynaptic neurons.
	isi = numpy.empty(0)	# Array to store interspike intervals of postsynaptic firings.
	for j in range(ensemble_size):
		...
		isi = numpy.append(isi, numpy.ediff1d(post.spikes)) 	# Dump interspike intervals into array.
	cv[i] = numpy.sqrt(numpy.var(isi))/numpy.average(isi)	# Compute the coefficient of variation of the ISI for the whole ensemble.


numpy.save('check_crpost.npy', cv)		# Store data.
