# Code to produce data for Figure 6: Postsynaptic neuron
...
class FHN():
	...
	def euler_maruyama(self):
		...
		self.v = numpy.full(self.N, self.v0)
		self.w = numpy.full(self.N, self.w0)
		#self.phase = numpy.random.uniform(-numpy.pi, numpy.pi, self.N)
		#self.phase=0
		vprevious = self.v
		...
		for i in range(int(self.T/self.dt)-1):
			...
			self.v=self.v + (self.v*(self.v-self.a)*(1-self.v)-self.w+self.A*numpy.sin(2*numpy.pi*self.f*i*self.dt))*self.dt/self.eps + (self.sigma/self.eps)*numpy.sqrt(self.dt)*dW
...
post.RK4()
numpy.save('check_postspikes.npy', post.spikes)